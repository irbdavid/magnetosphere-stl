...
pole singularities. When peeling is enabled, `earth.stl` has the same nominal
magnetopause wedge removed (90° between the X-Z and equatorial planes in the default
cut). The complementary `earth_insert.stl` carries the roots of the exposed random
field lines; it can be printed with them and glued into the main Earth piece.
Both Earth cuts are planar near the globe; the deformed current-sheet height map is
used for the larger magnetopause face.
...
output and its manifest. Open surfaces are clipped as surfaces and remain open.
For a self-contained test set, use a fresh output directory: `--overwrite` replaces
selected files but does not remove older, unrelated STLs. The manifest lists the
files generated by the current run.

When explicitly selected, the L-shell component defaults to equatorial seed radii
`L = 2, 4, 6, 8, 15, 30, 45, 100`.
At each
...
ridge. Above L=6, the same tubes are subtracted from their corresponding
L-shell, leaving matching half-round grooves in the shell surface. L=6 and inner
shells remain smooth by default. Change the threshold with `--tube-groove-minimum-l`. Use
...
field calculations in the normal case. If a groove subtraction produces an open mesh,
the grooved shell is retained with a warning for visual inspection.
...
```bash
uv run magnetosphere-stl --defaults --field-line-tubes --overwrite
```
...
GSM equatorial half-plane (`Z=0`, `Y>=2 RE`), excluding Earth and leaving clearance
from the X-Z cut plane. Accepted seeds are at least 6.6 RE apart by default;
dart throwing continues until the remaining gaps reject
...
magnetopause, and exported together as `random_field_lines.stl`. When the
magnetopause is peeled, the tubes are also clipped to its exposed sector: between
the X-Z polar-fan face and the deformed magnetic-equator convection face for the
standard cut, with 0.5–2 mm clearance from the latter to avoid grazing cuts.
Clipping fragments that do not touch the Earth insert are discarded. Configure their
...
Random lines replace L-shell surfaces and their groove-generating tubes in the
default presets. To include those components too, add `--field-line-tubes`; to
generate only the L-shell surfaces, use `--defaults --only l-shells`. Disable the
random lines with `--no-random-field-lines`.
...
uv run magnetosphere-stl --output output/random-field-lines \
  ...
  --random-field-line-spacing-re 7.5 \
...
```bash
uv sync
# Generate the standard cutaway with convection, polar fan, and random lines.
# L-shell surfaces and their field-line tubes are omitted:
uv run magnetosphere-stl --defaults

# Generate the same component set for a severe compressed storm case
...
uv run magnetosphere-stl --output output/quiet-solar-wind \
  ...
  --tail-x-min-re -50 --l-shells 2,4,6,8,15,30,45,100
uv run pytest
uv run ruff check .
```
...
Every component in a run receives the same immutable `ProjectConfig`. Each run
generates one uniquely named STL per selected component, except components omitted
by the quick-test clip, plus `setup.json`, which records the exact inputs and output
files. Existing generated files are protected unless the
...
magnetopause cutter uses the same Shue model and driving conditions as the exported
magnetopause.
...
height map is `resources/wave-texture.png`; lighter pixels displace the cut faces
farther and darker pixels remain nearer the original planes. The unrotated image is
projected onto the GSM X-Y plane: image left/right runs from the configured tail
plane (`X = -50 R_E` by default) to the bow-shock nose, while image top/bottom
runs from positive to negative Y. Its full luminance range is normalized to `[0, 1]`
before the configured amplitude is applied. For this experiment, the boundary fade
is disabled, so image values at the magnetopause, bow shock, and roll-stop edges
are retained.
...
`--magnetosheath-texture-boundary-fade-re` is currently unused while the fade is
commented out.
...
`bow-shock`, `convection`, `current-sheet`, `polar-field-lines`, `random-field-lines`,
`field-line-wedges`, and `l-shells`.
Selecting an optional tube or wedge component automatically enables it. A defaults
preset does not add other component groups when `--only` is used.
